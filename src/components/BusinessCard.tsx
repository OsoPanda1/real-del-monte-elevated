...
const BusinessCard = ({
  ...
  isPremium = false, rating = 4.2, phone, index = 0,
}: BusinessCardProps) => {
  return (
    <motion.div
      ...
      className={`group flex gap-4 p-4 rounded-2xl transition-all duration-300 cursor-pointer hover:-translate-y-1 ${
        isPremium
          ? "glass-card border border-gold/20 shadow-gold"
          : "glass-card hover:border-electric/20"
      }`}
    >
      <div className="relative w-24 h-24 rounded-xl overflow-hidden shrink-0">
        ...
        {isPremium && (
          <div className="absolute top-1 left-1 w-5 h-5 rounded-full bg-gold flex items-center justify-center">
            <Award className="w-3 h-3 text-night-900" />
          </div>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className="text-xs text-electric font-medium uppercase tracking-wide font-body">{category}</span>
        <p className="text-sm text-muted-foreground mt-1 line-clamp-2 font-body">{description}</p>
        <div className="flex items-center gap-3 mt-2">
          <div className="flex items-center gap-1">
            <Star className="w-3 h-3 text-gold fill-gold" />
            <span className="text-xs font-medium text-foreground font-body">{rating}</span>
          </div>
          {phone && (
            <div className="flex items-center gap-1 text-muted-foreground">
              <Phone className="w-3 h-3" />
              <span className="text-xs font-body">{phone}</span>
            </div>
          )}
          <ExternalLink className="w-3 h-3 text-muted-foreground ml-auto group-hover:text-electric transition-colors" />
        </div>
      </div>
    </motion.div>
  );
