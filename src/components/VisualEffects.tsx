...
export const ParallaxSection = ({ children, className = "" }: { children: ReactNode; className?: string }) => {
  ...
  return (
    <motion.section ref={ref} initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 1.2 }} className={className}>
      {children}
    </motion.section>
  );
};
      ...
      <motion.div className="relative text-center px-6 z-10" style={{ opacity }}>
        ...
        {label && (
          <motion.span initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
            className="font-body text-[10px] tracking-[0.5em] uppercase text-gold/60 block mb-4">{label}</motion.span>
        )}
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.3 }}
          ...
          <span className="animate-gradient-text text-glow-gold" style={{ backgroundImage: "linear-gradient(135deg, hsl(43,80%,55%), hsl(35,70%,65%), hsl(43,80%,55%), hsl(25,60%,50%), hsl(43,80%,55%))", backgroundSize: "200% 200%", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
            {title}
          </span>
        </motion.h1>
        {subtitle && (
          <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }}
            className="font-display text-lg md:text-xl text-platinum/60 italic mt-6 max-w-xl mx-auto">{subtitle}</motion.p>
        )}
      </motion.div>
    </section>
  );
