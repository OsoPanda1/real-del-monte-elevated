...
const DichoBlock = ({ dicho, index }: { dicho: typeof dichos[0]; index: number }) => (
  <motion.div
    ...
    className="glass-card rounded-xl p-5 sm:p-6 relative overflow-hidden group hover:-translate-y-1 transition-transform duration-300"
  >
    ...
    <p className="font-display text-base sm:text-lg text-foreground/90 italic leading-relaxed mb-3">"{dicho.text}"</p>
...
const LegendCard = ({ legend, index }: { legend: typeof legends[0]; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="grid md:grid-cols-5 gap-6 md:gap-8 items-center"
    >
      ...
      <div className={`md:col-span-3 ${isEven ? "" : "md:order-1"}`}>
        ...
        <h3 className="font-display text-2xl sm:text-3xl md:text-4xl tracking-tight text-gradient-gold mb-4">{legend.title}</h3>
        <div className="w-12 h-px bg-electric/40 mb-4" />
        <p className="font-body text-xs sm:text-sm text-muted-foreground leading-relaxed">{legend.summary}</p>
      </div>
    </motion.div>
  );
};
...
const Cultura = () => {
  return (
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <ImmersiveHero image={iglesiaImg} title="Cultura" subtitle="Leyendas, dichos, tradiciones y la memoria colectiva del pueblo" label="Identidad Viva">
        ...
        <StaggerContainer className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 max-w-4xl mx-auto">
          {traditions.map((t) => (
            <StaggerItem key={t.name}>
              <GlowCard color="gold" className="p-6 sm:p-8 text-center">
                ...
                <h3 className="font-display text-xl sm:text-2xl text-foreground mb-3">{t.name}</h3>
                <p className="font-body text-[11px] sm:text-xs text-muted-foreground leading-relaxed">{t.description}</p>
              </GlowCard>
            </StaggerItem>
          ))}
        </StaggerContainer>
